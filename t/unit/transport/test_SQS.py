...
class test_Channel:
    ...
    def setup_method(self):
        ...
        self.producer = messaging.Producer(
            self.channel, self.exchange, routing_key=self.queue_name
        )
    ...
    def test_region(self):
        ...
        connection = Connection(
            transport=SQS.Transport, transport_options={"region": "us-west-2"}
        )
        channel = connection.channel()
    ...
    def test_entity_name(self):
        ...
        assert (
            self.channel.entity_name("foo.bar-baz*qux_quux") == "foo-bar-baz_qux_quux"
        )
        assert self.channel.entity_name("abcdef.fifo") == "abcdef.fifo"

    def test_resolve_queue_url(self):
        queue_name = "unittest_queue"
        assert self.sqs_conn_mock._queues[
            queue_name
        ].url == self.channel._resolve_queue_url(queue_name)
    ...
    def test_delete_message(self, queue_name, message, new_q_url, monkeypatch):
        ...
        fake_sqs.delete_message.assert_called_once_with(
            new_q_url, message["ReceiptHandle"]
        )
    ...
    def test_envelope_payload(
        self, initial_payload, raw_text, message, q_url, expect_body
    ):
    ...
    def test_messages_to_python(self):
        ...
        for m in self.sqs_conn_mock.receive_message(
            QueueUrl=q_url, MaxNumberOfMessages=kombu_message_count
        )["Messages"]:
            m["Body"] = Message(body=m["Body"]).decode()
            kombu_messages.append(m)
        json_messages = []
        for m in self.sqs_conn_mock.receive_message(
            QueueUrl=q_url, MaxNumberOfMessages=json_message_count
        )["Messages"]:
            m["Body"] = Message(body=m["Body"]).decode()
    ...
    @pytest.mark.parametrize(
        "body",
        ["", "True", "False", "[]", "{}", "1", "1.0", "This is the body as a string"],
    )
    def test_messages_to_python_body_as_a_string(self, body):
        ...
        self.sqs_conn_mock.send_message(
            q_url, MessageBody=body, MessageAttributes=msg_attributes
        )
        ...
        formatted_messages = self.channel._messages_to_python(
            received_messages, self.queue_name
        )
    ...
    def test_redelivered(self):
        self.channel.sqs().change_message_visibility = Mock(
    ...
    def test_get_async(self):
        ...
        assert get_list_args[3] == self.channel.sqs().get_queue_url(self.queue_name).url
    ...
    def test_fetch_message_attributes(self, fetch_attributes, expected):
        ...
        ] = fetch_attributes  # type: ignore
        async_sqs_conn = self.channel.asynsqs(self.queue_name)
        assert async_sqs_conn.message_system_attribute_names == sorted(
            expected["message_system_attribute_names"]
        )
        assert (
            async_sqs_conn.message_attribute_names
            == expected["message_attribute_names"]
        )
    ...
    def test_drain_events_with_prefetch_none(self):
        ...
        # called?
        assert (
            expected_receive_messages_count
    ...
    def test_basic_ack(
        ...
        message = {"sqs_message": {"ReceiptHandle": "1"}, "sqs_queue": "testing_queue"}
        mock_messages = Mock()
    ...
    def test_basic_ack_with_mocked_channel_methods(
        self, basic_reject_mock, basic_ack_mock
    ):
        """Test that basic_ack calls the delete_message properly"""
        message = {"sqs_message": {"ReceiptHandle": "1"}, "sqs_queue": "testing_queue"}
        mock_messages = Mock()
    ...
    def test_basic_ack_without_sqs_message(self, basic_reject_mock, basic_ack_mock):
        """Test that basic_ack calls the delete_message properly"""
        message = {"sqs_queue": "testing_queue"}
        mock_messages = Mock()
        mock_messages.delivery_info = message
        self.channel.qos.append(mock_messages, 1)
        self.channel.sqs().delete_message = Mock()
    ...
    def test_basic_ack_invalid_receipt_handle(self, basic_reject_mock, basic_ack_mock):
        """Test that basic_ack calls the delete_message properly"""
        message = {"sqs_message": {"ReceiptHandle": "2"}, "sqs_queue": "testing_queue"}
        error_response = {
            "Error": {
    ...
    def test_message_attribute(self):
        ...
        self.producer.publish(
            message,
            message_attributes={
                "Attribute1": {"DataType": "String", "StringValue": "STRING_VALUE"}
            },
        )
