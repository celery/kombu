...
* ``hash_tag``: (str) Prefix keys(unacked_key,unacked_index_key,
                      unacked_mutex_key,global_keyprefix) with the hash tag
  used by Kombu
* ``socket_timeout``
...
class Channel(RedisChannel):
    ...
    def __init__(self, connection, *args, **kwargs):
        ...
        if self.hash_tag:
            self.unacked_key = f'{self.hash_tag}{self.unacked_key}'
            self.unacked_index_key = f'{self.hash_tag}{self.unacked_index_key}'
            self.unacked_mutex_key = f'{self.hash_tag}{self.unacked_mutex_key}'
