...
class Channel(RedisChannel):
    ...
    def _poll_error(self, conn, type, **options):
        if type == 'LISTEN':
            ...
            conn.parse_response(type)

    def close(self):
        self._closing = True
        if self._in_poll or len(self._in_poll_connections) != 0:
            try:
                for conn in self._in_poll_connections:
    ...
    def _connparams(self, asynchronous=False):
        ...
        if asynchronous:
            ...
            class ManagedConnection(redis.Connection):
    ...
    def subclient(self):
        ...
        # `ClusterPubSub` initializes the connection and connection pool when `execute_command` is called for the first time
        pubsub_client.ping()
