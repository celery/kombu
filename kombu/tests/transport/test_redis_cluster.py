...
from kombu.transport import redis_cluster  # noqa
...
class Client(object):
    ...
    def __init__(self, db=None, port=None, connection_pool=None, **kwargs):
        ...
        self.connection_pool = self._sconnection_pool(self)
        self.connection = self.connection_pool.get_connection_by_key('-')
    ...
    class _snodemanager(object):
        """Redis cluster with one node .."""
        def keyslot(self, key):
            return 1

    class _sconnection_pool(object):
        """Mock one connection"""

        def __init__(self, client):
            self.client = client
            self.nodes = self.client._snodemanager()

        def get_connection_by_key(self, key):
            conn = self.client._sconnection(self)
            conn.node = self.get_node_by_slot('_')
            return conn
...
class Channel(redis_cluster.Channel):
    Client = Client
...
class Transport(redis_cluster.Transport):
    Channel = Channel
...
class test_Channel(Case):
    ...
    @skip_if_not_module('rediscluster')
    def setUp(self):
    ...
    def _get_one_delivery_tag(self, n='test_uniq_tag1'):
    ...
    def test_do_restore_message(self):
        ...
        with patch('kombu.transport.redis_cluster.crit') as crit:
            self.channel._do_restore_message(
                pl2, 'ex', 'rkey', client,
            )
    ...
    def test_restore(self):
        ...
        with patch('kombu.transport.redis_cluster.loads') as loads:
            ...
            client.pipeline.assert_called_with(False)
    ...
    def test_qos_restore_visible(self):
        ...
        qos = redis_cluster.QoS(self.channel)
        ...
        qos._vrestore_count = 0
        client.setnx.side_effect = redis_cluster.MutexHeld()
        qos.restore_visible()
    ...
    #     with self.assertRaises(redis_cluster.Empty):
    ...
    def test_brpop_read_raises(self):
        ...
        with self.assertRaises(redis_cluster.Empty):
            self.channel._brpop_read(**{'conn': c.connection})
    ...
    def test_poll_error(self):
        ...
        self.channel._poll_error('BRPOP', c.connection)
    ...
    def test_invalid_database_raises_ValueError(self):
        ...
        client = self.channel.connection.client

        with self.assertRaises(ValueError):
            client.hostname = '%s:%s/%s' % (
                client.hostname,
                client.port or redis_cluster.DEFAULT_PORT,
                'dwqeq'
            )
    ...
    def test_new_queue_with_auto_delete(self):
        redis_cluster.Channel._new_queue(self.channel, 'george', auto_delete=False)
        self.assertNotIn('george', self.channel.auto_delete_queues)
        redis_cluster.Channel._new_queue(self.channel, 'elaine', auto_delete=True)
    ...
    def test_connparams_regular_hostname(self):
        self.channel.connection.client.hostname = (
            'redis-cluster://george.vandelay.com'
        )
        self.assertEqual(
            self.channel._connparams()['startup_nodes'][0]['host'],
            'george.vandelay.com',
        )
    ...
    def test_get_response_error(self):
        ...
        self.assertIs(redis_cluster.Channel._get_response_error(self.channel),
                      ResponseError)
    ...
    def test_register_with_event_loop(self):
        ...
        conn = Mock(name='conn')
        loop = Mock(name='loop')
        redis_cluster.Transport.register_with_event_loop(transport, conn, loop)
    ...
    def test_transport_on_readable(self):
        ...
        redis_cluster.Transport.on_readable(transport, 13)
        ...
        with self.assertRaises(KeyError):
            redis_cluster.Transport.on_readable(transport, 14)
    ...
    def test_transport_get_errors(self):
        self.assertTrue(redis_cluster.Transport._get_errors(self.connection.transport))

    @skip_if_not_module('rediscluster')
    def test_transport_driver_version(self):
        self.assertTrue(
            redis_cluster.Transport.driver_version(self.connection.transport),
        )
    ...
    def test_transport_get_errors_when_InvalidData_used(self):
        ...
        try:
            errors = redis_cluster.Transport._get_errors(self.connection.transport)
...
class test_RedisCluster(Case):

    @skip_if_not_module('redis')
    @skip_if_not_module('rediscluster')
    def setUp(self):
        self.connection = Connection(transport=Transport)
        self.exchange = Exchange('test_RedisCluster', type='direct')
        self.queue = Queue('test_RedisCluster', self.exchange, 'test_RedisCluster')
    ...
    def test_publish__get(self):
        ...
        producer = Producer(channel, self.exchange, routing_key='test_RedisCluster')
    ...
    def test_purge(self):
        ...
        self.assertEqual(channel._size('test_RedisCluster'), 10)
    ...
    def test_db_values(self):
        ...
        with self.assertRaises(Exception):
            Connection('redis-cluster://123.123.123.123:30001/foo').channel()
...
class test_MultiChannelPoller(Case):
    ...
    @skip_if_not_module('rediscluster')
    def setUp(self):
        self.Poller = redis_cluster.MultiChannelPoller
    ...
    def test_handle_event(self):
        ...
        p.handle_event(13, redis_cluster.READ)
        ...
        p.handle_event(13, redis_cluster.ERR)
        chan._poll_error.assert_called_with('BRPOP', conn)

        p.handle_event(13, ~(redis_cluster.READ | redis_cluster.ERR))
    ...
    def test_register_when_registered_reregisters(self):
        ...
        self.assertTupleEqual(p._fd_to_chan[10], (channel, conn, type))
        self.assertEqual(p._chan_to_sock[(channel, client, conn, type)], sock)
    ...
    def test_get_receives_multiple(self):
        ...
        channel._poll_error.assert_called_with('BRPOP', conn)
...
class test_Mutex(Case):
    ...
    def test_mutex(self, lock_id='xxx'):
        client = Mock(name='client')
        with patch('kombu.transport.redis_cluster.uuid') as uuid:
            # Won
            ...
            held = False
            with redis_cluster.Mutex(client, 'foo1', 100):
                held = True
            ...
            with self.assertRaises(redis_cluster.MutexHeld):
                held = False
                with redis_cluster.Mutex(client, 'foo1', '100'):
                    held = True
            ...
            client.setnx.return_value = True
            pipe.watch.side_effect = redis_cluster.redis.WatchError()
            held = False
            with redis_cluster.Mutex(client, 'foo1', 100):
                held = True
            self.assertTrue(held)
